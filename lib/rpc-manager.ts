...
// Local: health/status API - normalize SDK session stats into the shape the
// /api/health and SSE status_update events consume.
...
// Events that change the running/progress snapshot shown in the sidebar and
// health endpoint. On any of these we re-emit a status_update so clients don't
// have to poll get_state.
...
export class AgentSessionWrapper {
  ...
  // Local: build the LiveAgentStatus snapshot consumed by /api/health and the
  // SSE "connected"/"status_update" events. Distinct from the chat-facing
  // get_state payload - this one carries stats + retry state for monitoring.
  getLiveStatus(includeStats = true): LiveAgentStatus {
    ...
    const stats = rawStats ? normalizeStats(rawStats as LiveSessionStats) : undefined;
    const messageCount = stats?.totalMessages ?? 0;

    return {
      sessionId: this.inner.sessionId,
      sessionFile: this.inner.sessionFile ?? "",
      running: this.isRunning() || Boolean(this.inner.isRetrying),
  ...
  start(): void {
    this.unsubscribe = this.inner.subscribe((event: AgentEvent) => {
      ...
      notifyRunningChange();
      // Local: re-emit a status_update for events that change the live status
      // snapshot, so /api/health clients and the sidebar stay in sync without
      // polling get_state.
      if (STATUS_EVENT_TYPES.has(event.type)) {
        const includeStats = event.type !== "tool_execution_start";
        this.emit({ type: "status_update", status: this.getLiveStatus(includeStats) });
      }
    });
    this.resetIdleTimer();
    notifyRunningChange();
  ...
  async send(command: Record<string, unknown>): Promise<unknown> {
    ...
    switch (type) {
      ...
      case "get_state": {
        // Local getLiveStatus (health + sidebar) + upstream chat fields
        // (prompt/bash flags, queues, extension UI).
        const live = this.getLiveStatus(true);
        return {
          ...live,
      ...
      case "set_model": {
        ...
        if (!model) {
          await this.inner.modelRuntime.refresh();
