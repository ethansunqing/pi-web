...
async function exportMarkdown(id: string, filePath: string): Promise<Response> {
  ...
  const fileBaseName = sanitizeExportBaseName(exportContext.name);

  return new Response(markdown, {
    headers: {
      "Content-Type": "text/markdown; charset=utf-8",
      "Content-Disposition": getContentDisposition(`${fileBaseName}.md`, false),
...
export async function GET(
  ...
  try {
    ...
    if (format === "markdown") {
      return exportMarkdown(id, filePath);
    }
